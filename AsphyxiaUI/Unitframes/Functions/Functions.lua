...
hooksecurefunc( S, "PostUpdateHealth", function( health, unit, min, max )
	...
	elseif( C["global"]["globalstyle"] == "vethek" ) then
		if( not UnitIsConnected( unit ) or UnitIsDead( unit ) or UnitIsGhost( unit ) ) then
			if( not UnitIsConnected( unit ) ) then
				health.value:SetText("|cffFFFFFF" .. L.unitframes_ouf_offline .. "|r" )
			elseif( UnitIsDead( unit ) ) then
				health.value:SetText( "|cffFFFFFF" .. L.unitframes_ouf_dead .. "|r" )
			elseif( UnitIsGhost( unit ) ) then
				health.value:SetText( "|cffFFFFFF" .. L.unitframes_ouf_ghost .. "|r" )
			end
		else
			if( min ~= max ) then
				if( unit == "player" and health:GetAttribute( "normalUnit" ) ~= "pet" ) then
					health.value:SetFormattedText( "|cffFE0000%s|r |cffFFFFFF-|r |cffFE0000%s|r", S.ShortValue( min ), S.ShortValue( max ) )
				elseif( unit == "target" or ( unit and unit:find( "boss%d" ) ) ) then
					health.value:SetFormattedText( "|cffFE0000%s|r |cffFFFFFF-|r |cffFE0000%s|r", S.ShortValue( min ), S.ShortValue( max ) )
				elseif( ( unit and unit:find( "arena%d" ) ) or unit == "focus" or unit == "focustarget" ) then
					health.value:SetText( "|cffFE0000" .. S.ShortValue( min ) .. "|r" )
				else
					health.value:SetText( "|cffFE0000-" .. S.ShortValue( max - min ) .. "|r" )
				end
			else
				if( unit == "player" and health:GetAttribute( "normalUnit" ) ~= "pet" ) then
					health.value:SetText( "|cffFE0000" .. S.ShortValue( max ) .. "|r" )
				elseif( unit == "target" or unit == "focus" or unit == "focustarget" or ( unit and unit:find( "arena%d" ) ) ) then
					health.value:SetText( "|cffFE0000" .. S.ShortValue( max ) .. "|r" )
				else
					health.value:SetText( " " )
...
hooksecurefunc( S, "PreUpdatePower", function( power, unit )
	if( C["global"]["globalstyle"] == "vethek" ) then
		power:SetStatusBarColor( 954, 0, 0 )
	end
end )
...
hooksecurefunc( S, "PostUpdatePower", function( power, unit, min, max )
	if( C["global"]["globalstyle"] == "vethek" ) then
		if( unit == "player" ) then
			...
			power.value:SetTextColor( 254, 0, 0 )
		end
	end
end )
...
local CheckInterrupt = function( self, unit )
	if( C["global"]["globalstyle"] == "vethek" ) then
		...
		if( self.interrupt and UnitCanAttack( "player", unit ) ) then
			self:SetStatusBarColor( 1, 1, 1, 0.8 )
		else
			self:SetStatusBarColor( 0.08, 0.08, 0.08, 1 )
		end
	end
end

hooksecurefunc( S, "CheckCast", function( self, unit, name, rank, castid )
	if( C["global"]["globalstyle"] == "vethek" ) then
		CheckInterrupt( self, unit )
	end
end )

hooksecurefunc( S, "CheckChannel", function( self, unit, name, rank )
	if( C["global"]["globalstyle"] == "vethek" ) then
		CheckInterrupt( self, unit )
	end
end )
