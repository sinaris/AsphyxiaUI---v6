...
local function SetTabStyle( frame )
	...
	_G[chat .. "EditBox"]:Point( "TOPLEFT", AsphyxiaUILeftChatTab, 2, -2 )
	_G[chat .. "EditBox"]:Point( "BOTTOMRIGHT", AsphyxiaUILeftChatTab, -2, 2 )

	_G[chat.."EditBox"]:HookScript("OnShow", function()
		_G[chat .. "EditBox"].backdrop:ClearAllPoints()
		_G[chat .. "EditBox"].backdrop:SetAllPoints( AsphyxiaUILeftChatTab )
...
BNToastFrame:HookScript( "OnShow", function( self )
	self:ClearAllPoints()
	self:Point( "BOTTOMLEFT", AsphyxiaUILeftChatBackground, "TOPLEFT", 0, 3 )
end )
...
S.SetDefaultChatPosition = function( frame )
	if( frame ) then
		...
		if( id == 1 ) then
			...
			frame:Point( "TOPLEFT", AsphyxiaUILeftChatTab, "BOTTOMLEFT", 0, -2 )
			frame:Point( "BOTTOMRIGHT", AsphyxiaUILeftDataTextPanel, "TOPRIGHT", 0, 2 )
		elseif( id == 4 and name == LOOT ) then
			if( not frame.isDocked ) then
				frame:ClearAllPoints()
				frame:Point( "TOPLEFT", AsphyxiaUIRightChatTab, "BOTTOMLEFT", 0, -2 )
				frame:Point( "BOTTOMRIGHT", AsphyxiaUIRightDataTextPanel, "TOPRIGHT", 0, 2 )
