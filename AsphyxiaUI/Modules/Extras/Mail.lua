...
-- Credits: All credits goes to the original Author: Kemayo
...
if( C["intmodules"]["mail"] ~= true
	or IsAddOnLoaded( "QuickAuctions" )
	or IsAddOnLoaded( "AuctionProfitMaster" )
	or IsAddOnLoaded( "OpenAll" )
	or IsAddOnLoaded( "Postal" )
	or IsAddOnLoaded( "TradeSkillMaster_Mailing" ) ) then return end
...
function openMail( index )
	if( not InboxFrame:IsVisible() ) then
		return
		stopOpening( "|cffffff00" .. L.Mail_NEED )
	end

	if( index == 0 ) then
		MiniMapMailFrame:Hide()
		return
		stopOpening( "|cffffff00" .. L.Mail_COMPLETE )
	end
...
function onEvent( frame, event, arg1, arg2, arg3, arg4 )
	if( event == "UI_ERROR_MESSAGE" ) then
		if( arg1 == ERR_INV_FULL ) then
			stopOpening( "|cffffff00" .. L.Mail_STOPPED )
		elseif( arg1 == ERR_ITEM_MAX_COUNT ) then
			stopOpening( "|cffffff00" .. L.Mail_UNIQUE )
		end
	end
end
